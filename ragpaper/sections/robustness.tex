...
Of the six robustness checks: variance, RAPTOR (2/3 datasets), prompt
robustness, and sub-chunk sensitivity (3/3 cells in the recovery
direction) confirm the headline result. The long-form check qualifies
it as short-form-specific. The noise-vs-coherence check rules out one
form of the strongest version of our claim (paradox magnitude exceeds
noise) but is consistent with a weaker, qualitative version (paradox is
sign-distinct from noise on the multi-hop and biomedical regimes). We
incorporate both qualifications in \S\ref{sec:limitations}.
