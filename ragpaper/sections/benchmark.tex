...
\begin{description}
  \item[\texttt{adversarial\_coherence}] 40 hand-authored cases
        ...
        detection AUC against the matched control set
        (Table~\ref{tab:adv_auc}). Designed to dissociate which
        coherence signal catches which fragmentation pattern.
  \item[\texttt{coherence\_paradox}] the multi-dataset query set
        ...
        metric: \texttt{paradox\_drop} and \texttt{v2\_recovery}
        (Table~\ref{tab:multidataset}). Lets researchers test whether
        their own retrievers / generators reproduce the paradox.
  \item[\texttt{human\_faithfulness}] 500 double-annotated items with
        binary faithfulness, 1--5 correctness, 1--5 helpfulness, and
        Fleiss' $\kappa$ (Table~\ref{tab:kappa}). Target metric: NLI
        $\leftrightarrow$ human Spearman correlation
        (Table~\ref{tab:nli_human}). Lets follow-on work calibrate new
