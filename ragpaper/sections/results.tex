...
Table~\ref{tab:hcpc_main} reports the central behavioral result. The three
regimes are matched on retrieval similarity to within 0.01, so the
faithfulness differences they produce must originate elsewhere. We argue
this residual is context coherence.
...
aggressive refinement, while intervening on only 16.7\% of queries
(vs.\ 100\% for HCPC-v1). On PubMedQA, the recovery is smaller (0.5783 to
0.5893): the residual failure cannot be repaired downstream because the
coherence loss originates upstream, in an encoder-corpus mismatch that no
refinement policy can correct. The asymmetry itself is informative---it
predicts, correctly, that the paradox is most severe in the regime where
passage-level similarity is healthy but evidence coherence is not.
