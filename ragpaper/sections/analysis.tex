...
\paragraph{Dataset coverage.} Two datasets provide a useful contrast between
domain-matched and domain-mismatched settings, but do not cover multi-hop
reasoning, conversational QA, or open-domain generation tasks where context
coherence may play a different role.

\paragraph{Model scale.} Both generators are in the 7--8B parameter range.
Larger models may handle incoherent contexts differently---for example, by
being more robust to fragmentation or by hallucinating more confidently.
Our findings should be validated at larger scales before being applied to
production systems using 70B+ models.

\paragraph{No external method comparison.} We compare retrieval
configurations within our pipeline rather than against published methods
such as Self-RAG or CRAG. This is a deliberate design choice (our
contribution is identifying the coherence failure mode, not proposing a
state-of-the-art retrieval system), but it means we cannot claim that
HCPC-v2 is competitive with these methods on standard benchmarks. Future
work should evaluate whether coherence-aware gating improves faithfulness
when integrated into these systems.
...
supported by the context) or penalize paraphrases that are faithful but
lexically distant from the context. We did not conduct large-scale human
evaluation, though manual inspection of failure cases
(Appendix~\ref{app:failure}) confirms the NLI-identified patterns.
