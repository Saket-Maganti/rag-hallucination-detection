...
ablation in \S\ref{sec:rob:noise} does not support this strong form on
...
We evaluate three open-weight $7$--$8$B generators throughout. A
frontier-scale check using Llama-$3.3$-$70$B and Mixtral-$8\times7$B
via the Groq API was scoped but not run before submission; we mark this
as the highest-priority follow-up for the camera-ready revision. The
prediction the coherence account makes is that the paradox magnitude
should shrink at $70$B+ as the parametric prior becomes a more
plausible substitute for fragmented evidence, but should not invert.
