...
Section~\ref{sec:results} are therefore the recovery numbers
...
sensitivity table (\S\ref{sec:rob:topk}, Table~\ref{tab:topk}).
...
with our long-form non-result on QASPER (\S\ref{sec:rob:longform}).
...
Section~\ref{sec:results}. CCS still detects the failure (its
...
(\S\ref{sec:rob:frontier}) using Llama-$3.3$-$70$B and the $120$B
