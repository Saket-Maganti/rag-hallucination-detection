...
\paragraph{Result.}
% TRACKED-CHANGES (revision): PENDING NUMERIC FILL.
% Fill this table from results/revision/fix_01/paired_wilcoxon.csv after
% the full generation/NLI run completes. Do not claim causal support
% unless H1 is supported by the pre-registered thresholds.
\begin{table}[t]
\centering
...
\midrule
\textsc{SQuAD} & {[}pending{]} & {[}pending{]} & {[}pending{]} & {[}pending{]} & {[}pending{]} & {[}pending{]} \\
\bottomrule
\end{tabular}
\end{table}

% IF-SUPPORTED: If the pre-registered test supports H1, replace this
% paragraph with: "At fixed mean query similarity, HIGH-CCS triples
% produce significantly higher faithfulness than LOW-CCS triples..."
% and include Figure~\ref{fig:fix01_pairs}.
%
% IF-NULL: If H1 is not supported, replace causal language in the title,
% abstract, \S5 theory, and discussion. Use "predicts" rather than
% "drives", and frame the theory as a consistency check.
