...
The correlational evidence in Sections~\ref{sec:results}--\ref{sec:analysis}
...
\begin{table}[!htbp]
\centering
\caption{Attention-level differences (fragmented $-$ coherent) on matched
pairs, averaged across output tokens. Positive $\Delta H$ means diffuse
attention under fragmentation; negative $\Delta M_{\text{ret}}$ means
attention moved off the retrieved span. Values filled from
\texttt{results/mechanistic/summary.csv}.}
\label{tab:mech_delta}
\small
\begin{tabular}{lccc}
\toprule
Source of pairs & $n$ & $\Delta H$ & $\Delta M_{\text{ret}}$ \\
\midrule
Adversarial (disjoint)    & TBD & TBD & TBD \\
Adversarial (contradict)  & TBD & TBD & TBD \\
Adversarial (drift)       & TBD & TBD & TBD \\
HCPC-v1 vs HCPC-v2        & TBD & TBD & TBD \\
\bottomrule
\end{tabular}
\end{table}

Figure~\ref{fig:mech_layers} (placeholder;
\texttt{results/mechanistic/layer\_profile.pdf}) overlays per-layer
$\Delta M_{\text{ret}}$ for the four pair sources; P3 predicts the curves
peak around the same depth band.
...
than replaces, the behavioural results in §\ref{sec:results}.
