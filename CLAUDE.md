...
**Phase 4 (ChatGPT-review hardening) — DONE except top-k autopopulation.** All 6 code files + 4 new figures + 5 paper sections shipped. Top-k ablation running in background (PID 21170, ~50 min remaining). After it lands, ONE command (`python3 experiments/build_topk_table.py`) auto-populates Table tab:topk and the paper recompiles to ~71 pages. **PDF currently: 69 pages, 0 LaTeX warnings, 0 lint errors.**
