...
- [ ] `python3 experiments/build_headline_figure.py` — produces `ragpaper/figures/headline_frontier.{pdf,tex}`
- [ ] `python3 experiments/build_ccs_calibration.py` — produces `ragpaper/figures/ccs_calibration.{pdf,tex}`
- [ ] `python3 experiments/build_qualitative_example.py` — produces `ragpaper/figures/qualitative_paradox.tex`
- [ ] Wire the three new figures into the paper (one `\input{...}` line each in `results.tex` or `analysis.tex`)
- [ ] `cd ragpaper && pdflatex main && bibtex main && pdflatex main && pdflatex main` — clean build, all `??` and `Citation undefined` warnings resolved
...
See `submission/paper_metadata.yml` for canonical values. The big ones:
...
| Abstract | Use `ragpaper/sections/abstract.tex` verbatim; OpenReview strips LaTeX so paste the plain-text version (script: `python3 -c "import re; t=open('ragpaper/sections/abstract.tex').read(); print(re.sub(r'\\\\[a-zA-Z]+\\{?|\\$|\\}', '', t))"`). |
...
The tarball already excludes chroma_db, caches, and the ragpaper.zip
build snapshot. If the form wants a single PDF supplement, also include
`ragpaper/main.pdf` separately.
